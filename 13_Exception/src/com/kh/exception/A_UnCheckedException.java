...
public class A_UnCheckedException {
	 ...
	 * 
	 * - IndexOutOfBoundsExcepion	: 배열이나 리스트 등에 부적절한 인덱스로 접근 시 발생
	...
	public void test1() {
		...
		// int result = n1 / n2;
		// => 두번 째 입력값(n2)이 0인 경우 예외 발생. -> 프로그램이 비정상적으로 종료됨!
		...
		*/
		...
		try {
			...
			System.out.println(e.getMessage());	// 예외에 대한 설명(메세지
	...
	public void test3() {
		...
		try {
			...
			//    모든 자식 예외 발생 시 처리 가능
			System.out.println("인덱스가 잘못 되었거나, 배열 크기가 음수인 경우...");
		}	
		// => 각각의 예외 발생 시 실행할 동작이 다를 경우
		//	  다중 catch 블럭을 사용하여 예외 별로 세분화 할 수 있음!
		//	  이때, 순서에 유의해야 함! (부모타입보다 자식타입이 위쪽에 작성되어야 함)
